    ...
    <div className="rounded w-full  p-4 ring ring-indigo-50 sm:p-6 lg:p-8 bg-slate-50 hover:shadow-md cursor-pointer">
      <div className="flex items-center justify-between gap-4 sm:gap-8 mb-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <strong className="block text-base font-bold">{budget.name}</strong>
      ...
      <div className="flex flex-col w-full">
        <div className="flex justify-between mb-2">
          <p className="text-sm text-gray-500">${budget.totalSpend ?? '0'}{' spend'}</p>
          <p className="text-sm text-gray-500">${Calculte(Number(budget.amount),Number(budget.totalSpend))}{' remaining'}</p>
        </div>
        <div>
          <div className="bg-primary/20 h-2 rounded-full w-full">
            <div
              ...
              style={{
                width: `${(budget.totalSpend / Number(budget.amount)) * 100}%`,
              }}
            ></div>
          </div>
        </div>
